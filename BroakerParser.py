...
class Clear(Broaker):
    ...
    def process(self, page):
        ...
        for line in text.split('\n'):
            res = re.compile(r'[\w\d-]+\s(C|V)\s+(?:VISTA|FRACIONARIO)\s(?:\d\d\/\d\d)?([\w\d\s.\/]+?)\s\s+([\w\d\s#]+?)\s([\d.,]+)\s([\d.,]+)\s([\d.,]+)\s(\w)')\
                    .search(line)
            if res:
                # print (res.group(0))
                ...
                line_itens.append(order(code, Date, name, opType, category,\
                     int(res.group(4).replace('.','')), to_float(res.group(5)), to_float(res.group(6)), code.split(' ')[0] ))
                continue
...
class Clear_proventos():
    ...
    def process(self, page):
        ...
        line_itens = []
        for line in text.split('\n'):
            res = re.compile(r'([a-zA-Z/. ]+)\s([a-zA-Z/]+)\s([0-9,.]+)\s([0-9,.]+)\s([0-9,.]+)\s([0-9,.]+)\s([0-9/]+)').search(line)
            if res:
                # print (res.group(0))
